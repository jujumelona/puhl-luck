...
/// - Sparse logit generation (P53: hierarchical candidates, evidence scoring, top-k sampling)
...
fn puhl_luck_core(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    // Sparse logit generation (P53: hierarchical candidates + evidence scoring)
    sparse_logit::register_functions(m)?;
