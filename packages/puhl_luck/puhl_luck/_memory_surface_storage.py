...
Key principle:
- expose_pair(input, target) → store target with INPUT SIGNATURE (not raw text)
- generate(query) → field/operator activation → retrieve matching sequences → return raw text
- NO text comparison - use field/operator patterns only
...
All indexed by field/operator signatures, not raw text.
"""
...
class SurfaceSequence:
    """
    A raw surface sequence (target output text) indexed by input.
    
    This is what the system should output verbatim when queried
    with a matching input.
    """
    
    sequence_id: str  # stable hash of target
    raw_text: str  # NEVER MODIFIED - the actual output
    tokens: List[str]  # tokenized target (for token transitions)
    
    # INPUT INDEXING (for retrieval)
    source_input_hash: str  # hash of input text
    input_features: List[str]  # features from input
    input_tokens: List[str]  # tokens from input
    
    # Stats
    retrieval_count: int = 0
    successful_output: bool = True  # marked False if user corrects it
    was_retrieved_copy: bool = False  # True if output was exact retrieval (not generated)
...
class SurfaceSequenceStorage:
    ...
    Stores raw text indexed by INPUT SIGNATURE (field/operator activation):
    - operator_to_sequences: operator_id → sequences
    - concept_to_sequences: concept_id → sequences  
    - field_feature_to_sequences: field_feature → sequences
    
    NO raw text/token comparison for retrieval.
    """
    ...
    def __init__(self):
        ...
        # PRIMARY: Index by INPUT (for retrieval)
        ...
        self.target_token_to_sequences: Dict[str, Set[str]] = {}  # target_token → seq_ids (auxiliary)
    ...
    def retrieve_by_input_features(
        ...
        if candidate_ids:
            self.retrieval_stats["feature_overlap"] += 1
    ...
    def retrieve_by_input_tokens(
        ...
        if candidate_ids:
            self.retrieval_stats["token_overlap"] += 1
    ...
    def retrieve_by_target_tokens(
        ...
        if candidate_ids:
            self.retrieval_stats["target_overlap"] += 1
