...
class RecursiveStabilization:
    ...
    def __init__(
        ...
        self.convergence_threshold = float(convergence_threshold or 0.0)
        self.oscillation_window = int(oscillation_window or 0)
        self.damping_factor = float(damping_factor or 0.0)
    ...
    def _dynamic_max_iterations(self, context: InputContext) -> int:
        parts = [str(getattr(context, 'query_text', '') or ''), str(getattr(context, 'partial_output', '') or '')]
        feature_count = len(getattr(context, 'query_features', []) or [])
        token_like = sum(len(x.split()) for x in parts) + feature_count
        import math
        return max(1, int(math.ceil(math.sqrt(max(1, token_like + self.total_stabilizations + 1)))) + int(math.ceil(math.log2(max(2, feature_count + self.total_stabilizations + 2)))))

    def _dynamic_threshold(self, energy_history: List[float]) -> float:
        if self.convergence_threshold > 0.0:
            return float(self.convergence_threshold)
        if len(energy_history) < 2:
            return 0.0
        span = max(1e-12, max(energy_history) - min(energy_history))
        return span / max(1.0, float(len(energy_history) ** 2))

    def _dynamic_oscillation_window(self, energy_history: List[float]) -> int:
        if self.oscillation_window > 0:
            return int(self.oscillation_window)
        import math
        return max(1, int(math.ceil(math.sqrt(max(1, len(energy_history))))))

    def _dynamic_damping(self, energy_history: List[float]) -> float:
        if self.damping_factor > 0.0:
            return float(self.damping_factor)
        if len(energy_history) < 2:
            return 1.0
        import math
        volatility = abs(float(energy_history[-1]) - float(energy_history[-2]))
        return 1.0 / math.sqrt(1.0 + volatility)
    ...
    def stabilize(
        ...
        """
        max_iter = int(custom_max_iterations) if custom_max_iterations and int(custom_max_iterations) > 0 else (self.max_iterations if self.max_iterations > 0 else self._dynamic_max_iterations(initial_context))
        threshold = float(custom_threshold) if custom_threshold and float(custom_threshold) > 0.0 else self._dynamic_threshold([])
        ...
        for iteration in range(max_iter):
            ...
            osc_window = self._dynamic_oscillation_window(energy_history)
            if iteration >= osc_window:
    ...
    def detect_oscillation(self, energy_history: List[float]) -> bool:
        ...
        """
        osc_window = self._dynamic_oscillation_window(energy_history)
        if len(energy_history) < osc_window:
            return False
        
        recent = energy_history[-osc_window:]
