...
def looks_like_file_query(value: str) -> bool:
    text = str(value)
    if not text or len(text) > 260:  # Windows MAX_PATH limit
        return False
    
    # Multi-line text is not a file path
    if "\n" in text or "\r" in text:
        return False
    
    # Check for path separators (but not just ":" alone which appears in code)
    has_backslash = "\\" in text
    has_forward_slash = "/" in text
    has_colon = ":" in text
    
    # Only consider it a file path if:
    # - Has backslash or forward slash (actual path separators)
    # - Has colon AND is short (like "C:\path") not long code
    if has_backslash or has_forward_slash:
        # Try to check if it exists, catch OSError for too-long paths
        try:
            return Path(text).exists() or Path(text).suffix.lower() in ALL_FILE_SUFFIXES
        except (OSError, ValueError):
            return False
    
    if has_colon and len(text) < 100:  # Short text with colon might be drive path
        try:
            return Path(text).exists()
        except (OSError, ValueError):
            return False
    
    # Check file extension for simple filename
    try:
        suffix = Path(text).suffix.lower()
        return suffix in ALL_FILE_SUFFIXES and len(text.split()) == 1
    except (OSError, ValueError):
        return False
